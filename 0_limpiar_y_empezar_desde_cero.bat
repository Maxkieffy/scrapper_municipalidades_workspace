...
powershell -NoProfile -ExecutionPolicy Bypass -Command ^
  ...
  "if(Test-Path $pidFile){$p=[int](Get-Content $pidFile); if(Get-Process -Id $p -ErrorAction SilentlyContinue){Write-Error 'Hay un scrapeo activo. Detengalo antes de limpiar.'; exit 2}else{Remove-Item -LiteralPath $pidFile -Force}};" ^
...
if errorlevel 1 (
  echo Limpieza cancelada por seguridad.
  pause
  exit /b 1
)
